...
sys_env_set_status(envid_t envid, int status)
{
	...
	struct Env* op_env;
	int result = envid2env(envid, &op_env, 1);
	if (result < 0) {
		return -E_BAD_ENV;
	}
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	int ret = page_insert(op_env->env_pgdir, new_pginfo, va, perm);
	if (ret < 0) {
		page_free(new_pginfo);
		return -E_NO_MEM;
	}
	return ret;
}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	int flag = PTE_U | PTE_P;
	if((perm & ~(PTE_SYSCALL)) !=0 ||(perm & flag) != flag) {
		return -E_INVAL;
	}
	...
	pte_t* pte_store;
	struct PageInfo* pginfo = page_lookup(src_env->env_pgdir, srcva, &pte_store);
	if (pginfo == NULL) {
		return -E_INVAL;
	}
	...
	if (page_insert(dst_env->env_pgdir, pginfo, dstva, perm) < 0) {
    return -E_NO_MEM;
	}
	return 0;
}
...
sys_page_unmap(envid_t envid, void *va)
{
	...
	struct Env* op_env;
	if (envid2env(envid, &op_env, 1) < 0) {
		return -E_BAD_ENV;
	}
	pte_t* pte_store;
	struct PageInfo* pginfo = page_lookup(op_env->env_pgdir, va, &pte_store);
	if (pginfo == NULL) {
		return 0;
	}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if ((uintptr_t)srcva < UTOP) {
		...
		if((uintptr_t)(target->env_ipc_dstva) < UTOP){
			// check the target is receiving a page
			// insert page map
			if(page_insert(target->env_pgdir, pginfo, target->env_ipc_dstva, perm) < 0){
				return -E_NO_MEM;
			}
			// insert success
